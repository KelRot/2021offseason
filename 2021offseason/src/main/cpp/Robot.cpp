...
void Robot::AutonomousPeriodic() 
{
    ...
    else
    {
        if(phase == 1)
        {
            drive.ArcadeDrive(-encpid.computePID(-enc.GetDistance(), 280, 50), -gyropid.computePID(gyro.GetAngle(), 0, 10));
            if(t.Get() > 5 && abs(280 + enc.GetDistance()) < 5 && abs(encpid.errord)<1)
            {
                t.Reset();
                phase++;
            }
